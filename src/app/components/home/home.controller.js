function HomeController(handleDataService,handleAnswersService,handleQuestionService,$scope,$rootScope){
    const $ctrl = this;
    ...
    handleQuestionService.getAllQuestions().then(function (result) {
        ...
        var oneQuestion;
        for(oneQuestion of result){
            var newQuest = {
                id: oneQuestion.attributes.qNum,
                prompt: oneQuestion.attributes.text,
                answers: oneQuestion.attributes.Answers
            }
            $ctrl.questions.push(newQuest);
        }
